...
class AllocationApp:
    ...
    def _create_stage_item(self, idx, stage_id, name, desc):
        try:
            ...
            # 固定宽度140px的阶段卡片
            stage_frame = tk.Frame(self.stages_container, bg=bg_color, width=140)
            stage_frame.config(highlightbackground="#E5E7EB", highlightcolor="#E5E7EB", highlightthickness=1)
            stage_frame.pack(side=tk.LEFT, padx=(0 if idx == 0 else 8, 0))
            stage_frame.pack_propagate(False)  # 固定宽度
            
            stage_content = tk.Frame(stage_frame, bg=bg_color)
            stage_content.pack(fill=tk.BOTH, expand=True, padx=10, pady=12)
            ...
            stage_name_label = tk.Label(stage_content, text=name, font=("SF Pro Display", 11, "bold"), bg=bg_color, fg="#1F2937")
            stage_name_label.pack(pady=(0, 4))
            
            # 文字自动换行，宽度限制在120px
            stage_desc_label = tk.Label(stage_content, text=desc, font=("SF Pro Display", 9), bg=bg_color, fg="#6B7280", wraplength=120, justify=tk.LEFT)
