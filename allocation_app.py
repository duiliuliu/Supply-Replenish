...
class AllocationApp:
    ...
    def create_header(self, parent):
        ...
        title_label = ttk.Label(header_frame, text="加单商品分配系统", style="Title.TLabel")
    ...
    def create_config_section(self, parent):
        ...
        for i, level in enumerate(levels):
            ...
            try:
                val = str(self.config.get("allocation_config", {}).get("coverage_days", {}).get(level, 14))
            except:
                val = "14"
            entry.insert(0, val)
        ...
        for i, level in enumerate(levels):
            ...
            entry.pack(side=tk.TOP, ipady=8)
            try:
                val = str(self.config.get("allocation_config", {}).get("level_weights", {}).get(level, 1.0))
            except:
                val = "1.0"
            entry.insert(0, val)
        ...
        for i, level in enumerate(levels):
            ...
            entry.pack(side=tk.TOP, ipady=8)
            try:
                val = str(self.config.get("allocation_config", {}).get("safety_factors", {}).get(level, 0.3))
            except:
                val = "0.3"
            entry.insert(0, val)
        ...
        for i, level in enumerate(levels):
            ...
            entry.pack(side=tk.TOP, ipady=8)
            try:
                val = str(self.config.get("allocation_config", {}).get("min_target_inventory", {}).get(level, 0))
            except:
                val = "0"
            entry.insert(0, val)
    ...
    def create_button(self, parent, text, style="primary"):
        if style == "primary":
            btn = tk.Button(parent, text=text, font=("SF Pro Display", 12, "bold"), bg="#2563EB", fg="white", relief=tk.FLAT, padx=24, pady=10, cursor="hand2", activebackground="#1E40AF", activeforeground="white")
            ...
            btn = tk.Button(parent, text=text, font=("SF Pro Display", 12), bg="#F3F4F6", fg="#4B5563", relief=tk.FLAT, padx=24, pady=10, cursor="hand2", activebackground="#D1D5DB", activeforeground="#374151")
            btn.bind("<Enter>", lambda e, b=btn: b.config(bg="#D1D5DB"))
            btn.bind("<Leave>", lambda e, b=btn: b.config(bg="#F3F4F6"))
            btn.bind("<ButtonPress>", lambda e, b=btn: b.config(bg="#CBD5E1"))
            btn.bind("<ButtonRelease>", lambda e, b=btn: b.config(bg="#D1D5DB"))
        return btn
    ...
    def create_logic_section(self, parent):
        ...
        for idx, (stage_id, name, desc) in enumerate(stages):
            ...
            stage_frame.pack(fill=tk.X, pady=(0 if idx == 0 else 12, 0))
            ...
            name_label = tk.Label(content_frame, text=name, font=("SF Pro Display", 12, "bold"), bg=bg_color, fg=fg_color)
    ...
    def enable_run_button(self):
        self.run_btn.config(state=tk.NORMAL, bg="#2563EB", fg="white", cursor="hand2", command=self.run_allocation, activebackground="#1E40AF", activeforeground="white")
    ...
    def enable_save_button(self):
        self.save_btn.config(state=tk.NORMAL, bg="#2563EB", fg="white", cursor="hand2", command=self.save_result, activebackground="#1E40AF", activeforeground="white")
